...
department_courses = {
    "Computer Science": ["Data Structures", "Algorithms", "Introduction to Programming", "Machine Learning", "Computer Networks", "Operating Systems", "Database Systems", "Software Engineering", "Human-Computer Interaction", "Cybersecurity"],
    "Engineering": ["Circuit Analysis", "Thermodynamics", "Fluid Mechanics", "Statics", "Dynamics", "Material Science", "Electrical Systems", "Renewable Energy Systems", "Control Systems", "Signal Processing"],
    "Mathematics": ["Calculus", "Linear Algebra", "Differential Equations", "Probability", "Statistics", "Discrete Mathematics", "Real Analysis", "Numerical Methods", "Complex Analysis", "Topology"],
    "Physics": ["Mechanics", "Electromagnetism", "Quantum Physics", "Thermodynamics", "Optics", "Nuclear Physics", "Astrophysics", "Condensed Matter Physics", "Particle Physics", "Relativity"],
    "Chemistry": ["Organic Chemistry", "Inorganic Chemistry", "Physical Chemistry", "Analytical Chemistry", "Biochemistry", "Environmental Chemistry", "Industrial Chemistry", "Polymer Chemistry", "Theoretical Chemistry", "Medicinal Chemistry"],
    "Biology": ["Cell Biology", "Genetics", "Evolutionary Biology", "Ecology", "Microbiology", "Neuroscience", "Plant Biology", "Marine Biology", "Bioinformatics", "Developmental Biology"],
    "English": ["English Literature", "Creative Writing", "Linguistics", "American Literature", "Comparative Literature", "World Literature", "Literary Theory", "Rhetoric and Composition", "Children's Literature", "Poetry Writing"],
    "History": ["World History", "European History", "American History", "Ancient Civilizations", "Medieval Europe", "Modern History", "Historical Methods", "Public History", "History of Science", "Cultural History"],
    "Psychology": ["General Psychology", "Abnormal Psychology", "Developmental Psychology", "Cognitive Psychology", "Biopsychology", "Social Psychology", "Personality Psychology", "Clinical Psychology", "Forensic Psychology", "Industrial-Organizational Psychology"],
    "Economics": ["Principles of Economics", "Microeconomics", "Macroeconomics", "Econometrics", "International Economics", "Development Economics", "Behavioral Economics", "Environmental Economics", "Public Economics", "Game Theory"]
}
...
for i in range(num_lecturers + 1, num_lecturers + num_students + 1):
    ...
    accounts.append({
        ...
        "contact_info": unique_contacts[i - 1],  # Assuming unique_contacts is correctly indexed
        "name": fake.name(),
        "major": major,
...
lecturers_without_courses = set(range(1, num_lecturers + 1)) - set(lecturer_courses.keys())
...
for course_code in all_course_codes:
    while len(course_memberships[course_code]) < 10:
        ...
        if len(student_enrollments[student_id]) < 6 and course_code not in student_enrollments[student_id]:
            student_enrollments[student_id].append(course_code)
            course_memberships[course_code].append(student_id)

with open("insert_queries.sql", "w") as f:
    # Insert into Account
    f.write("INSERT INTO Account (account_id, email, password, account_type, contact_info, name) VALUES\n")
    account_values = [
    ...
    lecturer_values = [f"({lecturer['account_id']}, '{lecturer['department']}')" for lecturer in accounts if lecturer['account_type'] == "Lecturer"]
    f.write("INSERT INTO LecturerDetails (lecturer_id, department) VALUES\n" + ",\n".join(lecturer_values) + ";\n\n")
    ...
    # Insert into Course
    f.write("INSERT INTO Course (course_code, course_name, lecturer_id, semester) VALUES\n")
    course_values = [
